...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['edit_id'])) {
    ...
    $stmt = $conn->prepare("UPDATE bank_accounts_driver SET is_verified = ?, updated_at = NOW() WHERE id = ?");
...
<table id="example" class="display text-dark" style="min-width: 845px">
  ...
  <tbody>
    <?php
    $query = $conn->query("SELECT * FROM bank_accounts_driver ORDER BY id DESC");
    while ($row = $query->fetch_assoc()):
    ?>
    <tr>
      <td>
        <?php
        $sqlvc = "SELECT * FROM workers WHERE id = '".$row['driver_id']."'";
