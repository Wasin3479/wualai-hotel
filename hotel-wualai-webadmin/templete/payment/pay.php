...
<div class="row">
  ...
  <div class="card p-4 shadow-sm"> <center>
    ...
    <?php if($txn['payment_method']==='prompt_pay' || $txn['payment_method']==='mobile_banking' || $txn['payment_method']==='bbl_paygate' ): 
...
<?php if ($provider === 'bbl_paygate'): ?>
    <a class="btn btn-success" href="<?= $txn['checkout_url'] ?>/providers/card" target="_blank" >
        ชำระเงินด้วยบัตร
    </a>

<?php elseif ($provider === 'prompt_pay'): ?>
    <img src="pp.jpg" width="256" border="0" alt=""><br>
    <img src="<?= $qr_qr ?>" alt="QR Code" width="256"> <br>
    BUNTOOK APPLICATION
     <div id="countdown" class="fw-bold text-danger"></div> 
   <?php 
        // สร้าง DateTime object
 $dt = new DateTime($txn['datepay_a']);

// บวก 5 นาที
$dt->modify('+5 minutes');

// แสดงผลลัพธ์
$expires_atc=$dt->format('Y-m-d H:i:s'); // ผลลัพธ์: 2025-08-18 23:07:54
        ?>
<br>
<a href="<?= $qr_qr ?>" download="qr-code.png">
    <button type="button" class="btn btn-info">ดาวน์โหลดภาพ</button>
</a>
...
<?php elseif ($provider === 'wechatpay'): ?>
    <a class="btn-checkout" href="<?= $txn['qr_data'] ?>" target="_blank">
        <i class="fab fa-weixin"></i>   
        ชำระเงินด้วยบัตร
    </a>

<?php elseif ($provider === 'mobile_banking'): ?>

    <a class="btn-checkout" href="<?= $txn['qr_data'] ?>" target="_blank"> 
        <i class="fas fa-mobile-alt"></i>   
        ชำระเงินด้วยแอปธนาคาร 
    </a>
    <script>
        window.onload = function () {
            // รอเล็กน้อยเพื่อให้แน่ใจว่าหน้าโหลดก่อน
            setTimeout(function () {
                window.location.href = "<?= $txn['qr_data'] ?>";
            }, 100); // 100 มิลลิวินาที
        };
    </script>
