...
if (isset($_POST['add'])) {
    ...
    if (move_uploaded_file($_FILES["image_url"]["tmp_name"], $target_file)) {
        $stmt = $conn->prepare("INSERT INTO banners (type, title, description, image_url, link_url, start_date, end_date, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
        $stmt->bind_param("ssssssss", $_POST['type'], $_POST['title'], $_POST['description'], $urlfile, $_POST['link_url'], $_POST['start_date'], $_POST['end_date'], $_POST['status']);
        $stmt->execute();
...
if (isset($_POST['edit'])) {
    ...
    $stmt = $conn->prepare("UPDATE banners SET type=?, title=?, description=?, image_url=?, link_url=?, start_date=?, end_date=?, status=? WHERE id=?");
    $stmt->bind_param("ssssssssi", $_POST['type'], $_POST['title'], $_POST['description'], $urlfile, $_POST['link_url'], $_POST['start_date'], $_POST['end_date'], $_POST['status'], $_POST['id']);
    $stmt->execute();
...
?>
   ...
   <button class="btn btn-primary mb-3" data-toggle="modal" data-target="#addModal">+ เพิ่มแบนเนอร์</button>
      ...
      <table id="example" class="display text-dark" style="min-width: 845px">
            <thead>
                <tr>
                       <th>แสดง</th><th>ชื่อ</th><th>คำอธิบาย</th><th>ภาพ</th><th>ลิงก์</th>
