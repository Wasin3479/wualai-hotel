...
?></td>
    ...
    <div class="row mt-4">
  <div class="col-12">
    <p><strong>ตำแหน่งปัจจุบันของคนขับ:</strong></p>
    <iframe
      width="100%"
      height="300"
      style="border:0;"
      loading="lazy"
      allowfullscreen
      src="https://maps.google.com/maps?q=<?= $row['driver_current_lat'] ?>,<?= $row['driver_current_lng'] ?>&z=15&output=embed">
    </iframe>
  </div>
</div>
...
<nav>
  <ul class="pagination justify-content-start">
    <?php if ($page > 1): ?>
      <li class="page-item">
        <a class="page-link" href="super_admin.php?page=view_jobs&pagel=<?= $page - 1 ?>&search=<?= urlencode($search) ?>&limit=<?= $limit ?>">« ก่อนหน้า</a>
      </li>
    <?php endif; ?>

    <?php for ($i = 1; $i <= $totalPages; $i++): ?>
      <li class="page-item <?= ($i == $page) ? 'active' : '' ?>">
        <a class="page-link" href="super_admin.php?page=view_jobs&pagel=<?= $i ?>&search=<?= urlencode($search) ?>&limit=<?= $limit ?>"><?= $i ?></a>
      </li>
    <?php endfor; ?>

    <?php if ($page < $totalPages): ?>
      <li class="page-item">
        <a class="page-link" href="super_admin.php?page=view_jobs&pagel=<?= $page + 1 ?>&search=<?= urlencode($search) ?>&limit=<?= $limit ?>">ถัดไป »</a>
      </li>
    <?php endif; ?>
  </ul>
</nav>
