...
if (isset($_POST['update_status'])) {
    ...
    if (in_array($newStatus, $validStatuses)) {
        $stmt = $conn->prepare("UPDATE transactions_driver SET status = ? WHERE id = ?");
        ...
        Swal.fire({
            ...
            window.location.href = 'super_admin.php?page=approve_withdraw&status=driver';
        });
        </script>";
    } else {
        echo "
...
// 3. ดึงข้อมูลทั้งหมด
$result = $conn->query("SELECT * FROM transactions_driver WHERE transaction_type = 'WITHDRAW' ORDER BY created_at DESC");
?>
  ...
  <th>จัดการ</th>
    ...
    <tbody>
      <?php while ($row = $result->fetch_assoc()): ?>
        ...
        <br>
        <?php
          $sqlui = "SELECT * FROM drivers WHERE id = '".$row['wallet_id']."' ";
