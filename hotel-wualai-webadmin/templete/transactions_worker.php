...
if (isset($_POST['update_status'])) {
    ...
    if (in_array($newStatus, $validStatuses)) {
        $stmt = $conn->prepare("UPDATE transactions_worker SET status = ? WHERE id = ?");
        ...
        Swal.fire({
            ...
            window.location.href = 'super_admin.php?page=approve_withdraw&status=worker';
        });
        </script>";
    } else {
        echo "
...
// 3. ดึงข้อมูลทั้งหมด
$result = $conn->query("SELECT * FROM transactions_worker WHERE transaction_type = 'WITHDRAW' ORDER BY created_at DESC");
?>
  ...
  <th>จัดการ</th>
    ...
    <tbody>
      <?php while ($row = $result->fetch_assoc()): ?>
       ...
       <td>
        <?php  $bank_account_number=""; $account_name="";
          $sqlui = "SELECT * FROM bank_accounts_worker WHERE worker_id = '".$row['wallet_id']."' and is_verified = '1'";
$queryui = mysqli_query($conn,$sqlui);

while($resultui=mysqli_fetch_array($queryui,MYSQLI_ASSOC))
{
    echo $resultui["bank_name"];  $bank_account_number=$resultui["bank_account_number"]; $account_name=$resultui["account_name"];
}
          ?>
        </td>
         <td><?= htmlspecialchars($bank_account_number) ?></td>
          <td><?= htmlspecialchars($account_name) ?></td>
          <td><?= htmlspecialchars($row['wallet_id']) ?></td>
